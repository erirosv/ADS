...
CREATE (student:Individual:Student{name: students.studentName, id: students.studentId, graduated: toBoolean(students.graduated), year: toInteger(students.year)});
...
MERGE (div)<-[k:isUnder]-(c:Course {name: courses.courseName, courseCode: courses.courseCode, level: courses.level, credits: courses.credits})-[r:ownedBy]->(prog);
...
MERGE (course)<-[k:isBelongingTo]-(pc:ProgrammeCourses {studyYear: programmeCourses.studyYear, courseType: programmeCourses.courseType, academicYear: programmeCourses.academicYear})<-[r:isProviding]-(prog)
...
MERGE (examiner)-[r:isExamining]->(ci:CourseInstance {name: courseInstances.courseInstance, studyPeriod: courseInstances.studyPeriod, academicYear: courseInstances.academicYear})<-[k:isConductedAs]-(course);
